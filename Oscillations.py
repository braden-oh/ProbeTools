...
def read_agilent_binary(fname, use_segments=False, include_time_vector=True, include_datetime=False):
...
def get_power_spectrum(ys, dt):
    """Computes the FFT/power spectrum of a waveform
    Args:
        ys (array): y data
        dt (float): time step in seconds
    Returns:
        freq (array): x axis (frequency) values in Hertz
        power (array): power magnitude values in arbitrary units
    """
    ys = np.asarray(ys)
    N = len(ys)
    freq = np.fft.rfftfreq(N, dt)
    fft_vals = np.fft.rfft(ys)
    power = np.abs(fft_vals)**2
    return freq, power

#===================================================

def get_chunked_power_spectrum(ys, dt, n):
    """Breaks a waveform into n chunks and computes a
    FFT/power spectrum for each chunk.
    Args:
        ys (array): y data
        dt (float): time step in seconds
        n (int): number of chunks; must be a whole divisor of len(ys)
    Returns:
        freq (array): x axis (frequency) values in Hertz
        power_spectras (list): list of power magnitude values for each chunk
    """
    ys = np.asarray(ys)
    # Split the sample into n chunks
    samples = np.split(ys, n)
    N = len(samples[0])
    freq = np.fft.rfftfreq(N, dt)
    # Compute the power spectras
    power_spectras = []
    for sample in samples:
        fft_vals = np.fft.rfft(sample)
        power = np.abs(fft_vals)**2
        power_spectras.append(power)
    return freq, power_spectras
